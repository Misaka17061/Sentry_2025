...
void Chassis_MoveTransform(ChassisHandle_t* chassis_handle, fp32* chassis_vx, fp32* chassis_vy)
{
	...
	fp32 spin_compensation_angle = -15.54f;
	if(chassis_handle->ctrl_mode == CHASSIS_SENTRY | chassis_handle->ctrl_mode == CHASSIS_SPIN)
	{
		chassis_handle->spin_relative_angle = chassis_handle->platform_yaw_ecd_angle + spin_compensation_angle;
		sin_yaw = arm_sin_f32((chassis_handle->platform_yaw_ecd_angle - 20.65) / RADIAN_COEF);
		cos_yaw = arm_cos_f32((chassis_handle->platform_yaw_ecd_angle - 20.65) / RADIAN_COEF);
//		sin_yaw = arm_sin_f32(chassis_handle->spin_relative_angle / RADIAN_COEF);
//		cos_yaw = arm_cos_f32(chassis_handle->spin_relative_angle / RADIAN_COEF);
	}
	else
	{
...
void Mecanum_Calculate(ChassisHandle_t* chassis_handle, fp32 chassis_vx, fp32 chassis_vy, fp32 chassis_vw)
{
    ...
    wheel_rpm[3] = ( chassis_vx + chassis_vy - chassis_vw * rotate_ratio_fl) * wheel_rpm_ratio;
    wheel_rpm[0] = (-chassis_vx + chassis_vy - chassis_vw * rotate_ratio_fr) * wheel_rpm_ratio;
    wheel_rpm[2] = ( chassis_vx - chassis_vy - chassis_vw * rotate_ratio_bl) * wheel_rpm_ratio;
    wheel_rpm[1] = (-chassis_vx - chassis_vy - chassis_vw * rotate_ratio_br) * wheel_rpm_ratio;
...
//    }
		for (uint8_t i = 0; i < 4; i++)
		{
			if(RefereeSystem_RobotState_Pointer()->chassis_power_limit==100)           //?ù?Y??ó|1|?ê?3?μ2?í?×a?ù?μêy
			{
