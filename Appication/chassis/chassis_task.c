...
static void ChassisSteerMotorSendCurrent(int16_t motor1_cur, int16_t motor2_cur, int16_t motor3_cur, int16_t motor4_cur);
...
void Chassis_Task(void const *argument)
{
        ...
        for (uint8_t i = 0; i < 4; i++)             //PID控制
        {
            ...
            chassis_handle.chassis_motor[i].current_set = pid_calc(&chassis_handle.chassis_motor[i].pid,
						...
						chassis_handle.chassis_steer_motor[i].given_value	=	chassis_handle.steeringAngleTarget[i];
						chassis_handle.chassis_steer_motor[i].sensor.palstance = chassis_handle.chassis_steer_motor[i].motor_info->speed_rpm*6;
						Chassis_Steer_MotorControl(&chassis_handle.chassis_steer_motor[i]);
        }
        ...
        if(chassis_handle.ctrl_mode == CHASSIS_RELAX)     
        {
            for (uint8_t i = 0; i < 4; i++)
            {
                chassis_handle.chassis_motor[i].current_set = 0;
						  	chassis_handle.chassis_steer_motor[i].current_set = 0;
            }
        }
	      ...
	      ChassisMotorSendCurrent(chassis_handle.chassis_motor[0].current_set,
                                ...
                                chassis_handle.chassis_motor[3].current_set);       //轮输出
				
				ChassisSteerMotorSendCurrent(chassis_handle.chassis_steer_motor[0].current_set, 
																		 chassis_handle.chassis_steer_motor[1].current_set, 
																		 chassis_handle.chassis_steer_motor[2].current_set,
																		 chassis_handle.chassis_steer_motor[3].current_set);   //舵输出
...
uint16_t Chassis_MOVE_RATIO = 800;
static void ChassisSentryMode(void)
{
		Comm_NavCmd_t* NavCmd_info = NavCmd_Pointer();
		chassis_handle.vx = NavCmd_info->vx * Chassis_MOVE_RATIO;
		chassis_handle.vy = -NavCmd_info->vy * Chassis_MOVE_RATIO;
		chassis_handle.vw = 200;		//400
}
...
static void ChassisSteerMotorSendCurrent(int16_t motor1_cur, int16_t motor2_cur, int16_t motor3_cur, int16_t motor4_cur) 
{
    Motor_SendMessage(chassis_handle.chassis_can, MOTOR_5TO8_CONTROL_STD_ID, motor1_cur, motor2_cur, motor3_cur, motor4_cur);
}
