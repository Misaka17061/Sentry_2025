...
static void ComputerControl_PlatformModeSwitch(void);
/* 函数体 --------------------------------------------------------------------*/
		...
		switch(console.ctrl_mode)
		{
                ...
                else
                {
                    ...
                    console.chassis_cmd = CHASSIS_STOP_CMD;
						...
						else if(app_type == CHASSIS_APP || app_type == RADAR_APP)
						{
								if(PlatformInfo_Pointer()->mode != PLATFORM_INIT && PlatformInfo_Pointer()->mode != PLATFORM_RELAX)
								{
...
{ 
    ...
    else if(console.rc->sw2 == REMOTE_SWITCH_VALUE_UP)          
    {
        console.gimbal_cmd = GIMBAL_VISION_AIM_CMD;
		console.chassis_cmd = CHASSIS_SEPARATE_GIMBAL_CMD;
		console.platform_cmd = HUMAN_CONTROL_CMD;
...
{
		fp32 t = 0;
		if(robot_hurt_t.hurt_type == 0x00 && vision_data.state != 1)
			console.under_attack = 1;
		else
			console.under_attack = 0;
		if (console.under_attack == 1)
		{
				t++;
				if(t == 1000)
					console.under_attack = 0;
		...
		console.shoot_cmd = SHOOT_START_CMD;
		ComputerControl_PlatformModeSwitch();
...
static void ComputerControl_PlatformModeSwitch(void)
{
		if(vision_data.state == 1)
		{
				console.platform_cmd = COMPUTER_VISION_CONTROL_CMD;
				console.chassis_cmd = CHASSIS_SPIN_CMD;
		}
		else
		{
				console.platform_cmd = COMPUTER_RADAR_CONTROL_CMD;
