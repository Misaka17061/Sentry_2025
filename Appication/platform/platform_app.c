...
extern NavDecision_e Decision_handle;
pid_t platform_pid;
TD_conctrol td;
/* 私有函数原形 --------------------------------------------------------------*/
...
void PlatformAppConfig(void)
{
    ...
    platform_handle.platform_can  = &can1_obj;
		...
		pid_init(&platform_handle.yaw_motor.dpid.outer_pid, POSITION_PID, 850.0f, 0.0f,
             8.0f, 0.0f, 1.0f);/*调速*//*慢加P，抖加D*/ /*50,0,50*/
    pid_init(&platform_handle.yaw_motor.dpid.inter_pid, POSITION_PID, 850, 0.0f,
             15.0f, 0.0f, 1.0f);/*调力*/ /*1.5,0,20*/
	
		pid_init(&platform_handle.follow_pid, POSITION_PID, 300.0f, 20.0f,
             2.0f, 0.0f, 0.0f);
	
//		Encoder_Offset(platform_handle.platform_can,PLATFORM_MOTOR_YAW_MESSAGE_ID,11234);			//电机初始化位姿
