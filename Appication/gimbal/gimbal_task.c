...
static void GimbalHitFeedbackMode(void);
        ...
        switch (gimbal_handle.ctrl_mode)
        {
						...
						case GIMBAL_VISION_AIM:
            {
                GimbalVisionAimMode();              
						...
						case GIMBAL_HIT_FEEDBACK:
            {
                GimbalHitFeedbackMode();
            }break;
            default:
                break;
        }
...
static void GimbalCtrlModeSwitch(void)
{
    ...
    else if (gimbal_handle.console->gimbal_cmd == GIMBAL_VISION_AIM_CMD)
    {
		...
		else if (gimbal_handle.console->gimbal_cmd == GIMBAL_HIT_FEEDBACK_CMD )	
		{
		    gimbal_handle.ctrl_mode = GIMBAL_HIT_FEEDBACK;
		}	
}
...
static void GimbalFollowPlatformMode(void)
{
    ...
    gimbal_handle.pitch_motor.mode = ENCONDE_MODE;
		...
		gimbal_handle.yaw_motor.given_value = pid_calc(&gimbal_handle.follow_pid,-gimbal_handle.yaw_motor.sensor.relative_angle,0);
...
static void GimbalHitFeedbackMode(void)
{
    ...
    gimbal_handle.pitch_motor.mode = GYRO_MODE;
		...
		yaw_target = gimbal_handle.yaw_motor.sensor.gyro_angle - gimbal_info->yaw_ecd_angle + armor_id * 90;
		pitch_target = 0;
...
{  
		fp32 yaw_target=0;
			...
			else if (info->state == 0)
			{
				...
				VAL_ROLLBACK(yaw_target,-60,+60,yaw_speed,0.2f);	
				VAL_ROLLBACK(gimbal_handle.pitch_motor.given_value,10,29,speed_head,0.3f);	
			}
}
...
static void GimbalVisionAimMode(void)
{   
//		if(gimbal_handle.last_ctrl_mode != GIMBAL_VISION_AIM)
//		gimbal_handle.yaw_motor.given_value = 0;
    gimbal_handle.yaw_motor.mode = GYRO_MODE;                                       //使能云台yaw轴陀螺仪模式
    gimbal_handle.pitch_motor.mode = ENCONDE_MODE;                                  //使能云台pitch轴编码器模式
    ...
    VAL_LIMIT(gimbal_handle.pitch_motor.given_value, gimbal_handle.pitch_motor.min_relative_angle, gimbal_handle.pitch_motor.max_relative_angle);//pitch角度限位
    gimbal_handle.yaw_motor.given_value  =  AngleTransform(gimbal_handle.yaw_motor.given_value, gimbal_handle.yaw_motor.sensor.gyro_angle);
		//VAL_LIMIT(gimbal_handle.yaw_motor.given_value, gimbal_handle.platform_yaw_ecd_angle - 60, gimbal_handle.platform_yaw_ecd_angle + 60);	
}
...
static void VisionDataUpdate(void)
{
    ...
    vision_data.yaw = FourBytesToFloat(info->yaw);
		...
		vision_data.palstance = FourBytesToFloat(info->palstance);
		vision_data.palstance = 180 * vision_data.palstance / PI;	
...
static void Aim_contorl(AutoAim_t* Aim)
{
		...
		switch (Aim->aim_mode)
    {
        ...
        yaw_vision_pid.i=0.00f;		
			...
			{
				 //自瞄开始，此时还未瞄准到规定角度差
         vision_data.yaw_success = AIM_NO,yaw_vision_pid.p = 0.012;//user set
        ...
        yaw_vision_pid.p= 0.01;//user set   
			}break;
 ...
 if(Aim->enable_paramSA)
 {
  ...
  yaw_vision_pid.p -= (Aim->Sp_parm/(float)(Aim->stay_time));
	...
	if(vision_data.yaw==-90)//90位规定的角度
		yaw_vision_pid.i=0;
  VAL_LIMIT(yaw_vision_pid.p,0.002f,0.05f);
