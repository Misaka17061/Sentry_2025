...
ReceiveHandle_t gimbal_rx_handle;
...
void GimbalAppConfig(void)
{
	...
	gimbal_handle.yaw_motor.offset_ecd = 3481;	//4701	3681	3481
    gimbal_handle.pitch_motor.offset_ecd =5585;  //5376		5585
    ...
    pid_init(&gimbal_handle.yaw_motor.dpid.outer_pid, POSITION_PID, 2000.0f, 0.0f,
             15.0f, 0.0f, 4.0f);/*调速*//*慢加P，抖加D*/ /*60,0,40*/
	...
	pid_init(&yaw_vision_pid,POSITION_PID, 200.0f, 200.0f,0.022, 0, 0.4); 	//d0.4
	pid_init(&pitch_vision_pid, POSITION_PID, 200.0f, 200.0f,0.025, 0, 0.4); 
...
static void Aim_init(AutoAim_t* Aim)
{
    ...
    Aim->stay_time = 0;
    Aim->systeam_time = 0;
    Aim->tol_angle = 0.5f;  //人为规定消抖角度范围(-tol_angle,tol_angle)
