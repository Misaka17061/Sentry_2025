    ...
    .ESO =
 	{
		...
		.k=8.f,
 		.Omega = 200.f,            //20.0   过大会引起系统稳定后的再振荡
 	},
 	.NLC =
 	{
 		.B = 2000.00f,   //1.92      //影响ESO的Z2，过大会延迟响应，系统输出更稳定，过小会快速响应，但会振荡或受噪音影响
 		.Beta_p = 0.f,          //加入积分项后响应速度更快
 		.Beta_n = 43.f,
 		.Beta_f = -0.01f,
...
static void _adrc_ESO_cal(ADRC * adrc)
{
	...
	b3 = adrc->ESO.k*adrc->ESO.Omega*adrc->ESO.Omega;//wo
	obLimit = adrc->NLC.MaxOutputClamp * adrc->NLC.B;
	e = adrc->ESO.Z1 - adrc->Measure;
	//adrc->ESO.Z1 += adrc->Time.Dtime *(adrc->ESO.Z2 - b1 * zepi_fal(e, adrc->ESO.Alpha1, adrc->ESO.Delta));//b1*e,belta01=b1
	adrc->ESO.Z1 += adrc->Time.Dtime *(adrc->ESO.Z2 - b1 * e);//b1*e,belta01=b1
	...
	adrc->ESO.Z2 += adrc->Time.Dtime *(adrc->ESO.Z3 - b2 * e+ adrc->NLC.B * adrc->NLC.U);
	...
	adrc->ESO.Z3 -= adrc->Time.Dtime *(b3 * e);
...
{
	adrc->NLC.E1 = adrc->TD.R1 - adrc->ESO.Z1;//闭环成功E1=0
	adrc->NLC.E2 = adrc->TD.R2 - adrc->ESO.Z2;//闭环成功E2=0
	adrc->NLC.E0 += adrc->NLC.E1 * adrc->Time.Dtime;//闭环成功E0稳定保持
	adrc->NLC.U0 = //adrc->NLC.Beta_p * zepi_fal(adrc->NLC.E0, adrc->NLC.Alpha_p, adrc->NLC.Delta) +
				   //adrc->NLC.Beta_n * zepi_fal(adrc->NLC.E1, adrc->NLC.Alpha_n, adrc->NLC.Delta)//根据误差大小使用三个非线性增益*三个权重累加得出输出
				   adrc->NLC.Beta_n * adrc->NLC.E1 +
				   adrc->NLC.Beta_f * adrc->NLC.E2;
...
void _adrc_reset_allParam(ADRC * adrc, ADRC_Init adrc_init) //参数初始化
{
	memset(adrc, 0, sizeof(&adrc));
	adrc->TD.R = adrc_init.TD.R;
	adrc->NLC.B = adrc_init.NLC.B;
...
ADRC zepi_create_ADRC(void)   //函数指针初始化
{
	ADRC adrc = {0};
