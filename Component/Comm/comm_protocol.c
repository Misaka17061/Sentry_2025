...
void Comm_ReceiveData(ReceiveHandle_t* p_handle, uint8_t* p_data, uint16_t len)
{
    if (p_handle == NULL)
		...
		if(fifo_s_free(&p_handle->fifo)<100)
				fifo_s_flush(&p_handle->fifo);
...
/*************************************************
 * Function: Comm_TransmitData_Computer
 ...
 * Return: 无
*************************************************/
void Comm_TransmitData_Computer(TransmitHandle_t* p_handle, uint8_t* p_data, uint16_t len)
{
    if (p_handle == NULL)
        return;

    if (fifo_s_free(&p_handle->fifo) > len + 2)
    {
        append_crc16_check_sum(p_data, len + 2);
        fifo_s_puts(&p_handle->fifo, (char*)p_data, len + 2);
    }
}
