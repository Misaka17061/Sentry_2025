...
typedef enum
{
    ...
    RADAR_MARK_DATA_CMD_ID      = 0x020C,       //雷达标记进度数据，固定以 1Hz频率发送
		...
		CLIENT_CUSTOM_DATA_CMD_ID   = 0x0301,       //机器人交互数据，发送方触发发送，频率上限为 10Hz
...
typedef struct     //比赛机器人状态：0x0201。发送频率：10Hz
{
    ...
    uint16_t chassis_max_power_limit;               //机器人最大底盘功率， 单位 w
    /* 主控电源输出情况： */
...
typedef struct         //交互数据 机器人间通信：0x0301。内容 ID:0x0200~0x02FF.发送频率：上限 10Hz
{
  uint16_t data_cmd_id;
	...
	uint8_t data[113];          //数据段 n 需要小于 113
} robot_interactive_data_t;

typedef struct         //键鼠遥控数据,0x0304
{
	int16_t mouse_x;
	int16_t mouse_y;
	int16_t mouse_z;
...
#define REFEREE_SYSTEM_FIFO_SIZE    (1024u)
...
ext_robot_hurt_t* RefereeSystem_HeatData_Pointer(void);
remote_control_t* RefereeSystem_ClientData_Pointer(void);
ext_game_state_t* RefereeSystem_GameState_Pointer(void);
ext_game_result_t* RefereeSystem_GameResult_Pointer(void);
ext_game_robot_HP_t* RefereeSystem_GameRobot_HP_t_Pointer(void);
ext_bullet_remaining_t* RefereeSystem_GameRobot_bullet_remaining_t_Pointer(void);
